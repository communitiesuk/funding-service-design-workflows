...
echo $S3_CREDENTIALS
echo $S3_CREDENTIALS | jq -r '.credentials'
echo $S3_CREDENTIALS | jq -r '.credentials.aws_access_key_id'

export AWS_ACCESS_KEY_ID=$(echo "${S3_CREDENTIALS}" | jq -r '.credentials.aws_access_key_id')
export AWS_SECRET_ACCESS_KEY=$(echo "${S3_CREDENTIALS}" | jq -r '.credentials.aws_secret_access_key')
export BUCKET_NAME=$(echo "${S3_CREDENTIALS}" | jq -r '.credentials.bucket_name')
export AWS_DEFAULT_REGION=$(echo "${S3_CREDENTIALS}" | jq -r '.credentials.aws_region')
